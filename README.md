# CoScientist V4 â€” deterministic core + pre-specified analysis engine
...
research loop that would drive these pieces â€” candidate generation, G2 novelty
retrieval and the cycle orchestrator â€” is not yet built.
...
`run: echo "TODO: orchestrator."` â€” invalid YAML, which GitHub would have
...
that exists, and every `[GUARANTEE: â€¦]` reference in these documents must
...
```bash
pip install -e ".[dev]"
pytest -q            # 286 tests, no credentials needed
```
...
number* â€” so the following was possible, and nothing in the system could see it:

```
design freezes â†’ outcomes visible â†’ edit 03_primary_models.R â†’ try another
specification â†’ edit again â†’ keep the one that works â†’ collect()
```
...
must be acknowledged `[GUARANTEE: UNSANCTIONED_IO_IS_DECLARED]` â€” unsanctioned
acquisition becomes a declared fact instead of an invisible one.

## The analysis engine

```
GMS manifests/catalog â”€â”€â–¶ selected frozen bytes/query extract â”€â”€â–¶ freeze-verify
                                                    â”‚
                                                    â–¼
                                      analysis-verify â”€â”€â–¶ R / Python
                                                                        â”‚
                                                                 streams (JSONL)
                                                                        â”‚
                                                                    collect â”€â”€â–¶ bridge
                                                                        â”‚
                                                         strict provenance â”€â”€â–¶ Drive
```
...
Stan deliberately has no runner of its own â€” a `.stan` file is a model, not an
...
`[GUARANTEE: LARGE_DATASETS_ARE_STREAMED]` â€” the 25 GB per-study ceiling was
...
collector and then uploaded to the authoritative folder anyway â€” undoing the
...
do not print numbers, they emit them â€” one JSON object per reportable quantity,
...
self-reported: a record could name a different â€” real, hashed â€” script as its
...
file containing half an analysis, and those records are real â€” which is exactly
...
arguments passed in the wrong order â€” a mistake whose output is a table that
...
freeze hash `[GUARANTEE: RANDOMNESS_IS_DERIVED_FROM_THE_DESIGN]` â€” a seed the
...
The R stack is declared in `R/dependencies.R` with a reason per package â€”
...
once set â€” nor can the design it marks be edited around it
...
`[GUARANTEE: UNITS_GATE_NUMERIC_MATCHING]` â€” 241 USD once satisfied a claim of
2.41 pp by division. Association is written down, not inferred from proximity,
because proximity picks the wrong record in "2.41 pp and âˆ’0.312 pp (p < 0.001)"
...
Numbers that are legitimately not results â€” Î±, a rate quoted from a cited paper
â€” cite the literal registry instead, where each has an id and a source. The allow-list is refused
...
sentence true â€” strict mode happily rendered "The p-value was
...
is `[GUARANTEE: PROSE_AND_CITATION_MUST_AGREE]`. Prefer the named macros â€”
`[[EST:t]]`, `[[CI:t]]`, `[[P:t]]`, `[[N:t]]`, `[[SE:t]]`, `[[CLUST:t]]` â€”
...
`[GUARANTEE: MANIFEST_REQUIRES_ITS_INTEGRITY_HASHES]` â€” the identical
...
is not a census of sources â€” admissible sources exist that carry no secret at
...
G4 recomputes what C716 discovered too late: an MDE of 6.5â€“7pp against a shock
...
killing inside a 3Ã— margin
`[GUARANTEE: CHEAP_SCREEN_DOES_NOT_ADJUDICATE_CLOSE_CALLS]` â€” a generic
...
on â€” above the outcome lock only
...
mid-execution â€” an LLM is never in the middle of anything, only at a boundary
...
Resources are metered on their own clocks â€” a monthly BigQuery cap, a per-cycle
...
src/coscientist/
  ...
  power.py                   G4 â€” the minimum detectable effect
...
`[GUARANTEE: EVERY_MODULE_IMPORTS]` â€” `bq.py` kept a stale import across two
...
`[GUARANTEE: ANALYSIS_STAGES_RUN_IN_ORDER]` â€” an earlier version verified
