...
The V4.7.0 tree currently collects **432 tests**. The suite covers package integrity, paper-local lifecycle compatibility, multi-paper concurrency/isolation, Director routing, explicit owner withdrawal, freeze/AnalysisLock/provenance contracts, Astra V3 repair invariants, outcome-blind Astra pre-freeze design repair primitives, current-period cross-taxonomy SEC fact selection, literal security-identifier preservation (including ticker `NA`), literal-reader recursion protection, and adversarial integrity gates.
