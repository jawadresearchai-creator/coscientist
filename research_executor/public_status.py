...
ALLOWED_TASK_TYPES = {
    ...
    "METHODOLOGY", "PUBLICATION", "ADVERSARIAL_REVIEW"
}
...
def build_public_status(job_id: str, task_type: str = "SMOKE_TEST") -> dict[str, Any]:
    validate_job_id(job_id); validate_task_type(task_type)
    now = utc_now()
    ...
    return {
        "schema_version":PUBLIC_SCHEMA_VERSION,"job_id":job_id,"task_type":task_type,
        "status":"SUCCEEDED","privacy_class":"PUBLIC_EXECUTOR_SAFE",
        "executor":"GITHUB_ACTIONS_PUBLIC","started_at":now,"completed_at":now,
        "deterministic_receipt":digest,
        "run":{
            "repository":os.getenv("GITHUB_REPOSITORY"),"run_id":os.getenv("GITHUB_RUN_ID"),
            "run_attempt":os.getenv("GITHUB_RUN_ATTEMPT"),"sha":os.getenv("GITHUB_SHA"),
        },
        "checks":checks,
    }

def validate_public_status(status: dict[str, Any]) -> None:
    required={"schema_version","job_id","task_type","status","privacy_class","executor",
              "started_at","completed_at","deterministic_receipt","run","checks"}
    missing=required-set(status)
    if missing: raise ValueError(f"public status missing required fields: {sorted(missing)}")
    if status["schema_version"] != PUBLIC_SCHEMA_VERSION: raise ValueError("unexpected schema version")
    validate_job_id(status["job_id"]); validate_task_type(status["task_type"])
    if status["privacy_class"]!="PUBLIC_EXECUTOR_SAFE": raise ValueError("public artifact must be PUBLIC_EXECUTOR_SAFE")
    if status["status"]!="SUCCEEDED": raise ValueError("public success receipt expects SUCCEEDED")
    forbidden_keys={
    ...
    if bad: raise ValueError(f"private/sensitive fields present in public status: {sorted(bad)}")

def write_public_status(job_id: str, output: str | Path, task_type: str = "SMOKE_TEST") -> Path:
    status=build_public_status(job_id,task_type); validate_public_status(status)
